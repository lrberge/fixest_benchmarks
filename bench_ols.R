...
bench_ols_large <- run_benchmark(
  dgps = data.table::rowwiseDT(
    dgp_name=, n_iters=, n_obs=, dgp_function=,
    "simple",     5L, 1e6, list(\() base_dgp(n = 1e3, type = "simple")),
    "difficult",  5L, 1e6, list(\() base_dgp(n = 1e3, type = "difficult")),
    "simple",     3L, 5e6, list(\() base_dgp(n = 5e3, type = "simple")),
    "difficult",  3L, 5e6, list(\() base_dgp(n = 5e3, type = "difficult"))
  ),
  estimators = data.table::rowwiseDT(
    est_name=, n_fe=, func=,
    "pyfixest.feols", 2L, list(\(df) {
